...
function mostrar(req,res) {
    ...
    if(!req.body.vinos) return res.status(404).send({message:'No se encontró el producto'});
...
function eliminar(req,res) {
    ...
    req.body.vinos[0].remove().then(vinosElim => {
        res.status(200).send({message:'Eliminado correctamente',vinosElim});
    }).catch(error=>res.status(500).send({error}));
}
